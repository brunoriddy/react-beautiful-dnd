...
export default function TaskResult({ task, result, pinned }) {
    ...
    if (result.type === 'error') {
        return React.createElement(ErrorResultView, { key: task.taskId, task: task, result: result });
    }
    if (result.type === 'static') {
        invariant(task.type === 'static', `Unexpected task type: ${task.type}`);
        const pin = pinned && pinned.type === 'static' ? pinned : null;
        return React.createElement(StaticResultView, { key: task.taskId, task: task, result: result, pinned: pin });
    }
    if (result.type === 'timed') {
        ...
        return React.createElement(TimedResultView, { key: task.taskId, task: task, result: result, pinned: pin });
    }
